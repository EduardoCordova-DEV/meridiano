...
for (const language of ['es', 'en'] as const) {
  test(`caso opcional ${language}: badge, edición, persistencia y contraste`, async ({ page }, testInfo) => {
    ...
    await expect(card.locator('.day-icon')).toHaveCount(0)
    await expect(card.locator('.case-badge')).toHaveCSS('font-size', '13px')
    await expect(card.locator('.client-label')).toHaveCSS('font-size', '13px')
    await expect(card.locator('.card-date')).toHaveCSS('font-size', '12px')
    await expect(card.locator('.card-offset')).toHaveCSS('font-size', '12px')
    ...
    const badgeBounds = await card.locator('.case-badge').boundingBox()
    const controlBounds = await card.locator('.card-actions button').first().boundingBox()
    expect(badgeBounds!.x + badgeBounds!.width).toBeLessThan(controlBounds!.x)
