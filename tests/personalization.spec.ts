...
for (const theme of ['light', 'dark'] as const) {
  test(`personalización ${theme}: todas las paletas, accesibilidad, conversión y persistencia`, async ({ page }, testInfo) => {
    ...
    await dialog.getByRole('button', { name: t.presetNames.xbox25, exact: true }).click()
    ...
    await expect(page.locator('html')).toHaveCSS('background-color', rgb(PRESETS.xbox25[theme === 'light' ? 'dark' : 'light'].background))
    ...
    await page.getByRole('dialog').getByRole('button', { name: t.addClock }).click()
    await page.reload()
    await expect(page.locator('html')).toHaveCSS('background-color', rgb(PRESETS.xbox25[theme].background))
    ...
    expect(stored.personalization[theme]).toEqual(PRESETS.xbox25[theme])
    expect(stored.palettePreset).toBe('xbox25')
