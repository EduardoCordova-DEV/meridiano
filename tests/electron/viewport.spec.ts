...
async function expectUniformCards(page: Page) {
  expect(await page.evaluate(() => {
    ...
    const height = Math.min(width * (innerHeight <= 900 ? .75 : .8), host.clientHeight - 38)
    ...
    return {
      ...
      errors: cards.flatMap((card) => {
        ...
        const failures = []
        if (Math.abs(rect.width - width) > 1 || Math.abs(rect.height - height) > 1) failures.push({ expected: { width, height }, actual: { width: rect.width, height: rect.height } })
        ...
        const pause = card.querySelector('.celestial-toggle')!.getBoundingClientRect()
        const edit = card.querySelector('.card-actions button')!.getBoundingClientRect()
        if (pause.right >= edit.left || Math.abs(pause.top - edit.top) > 1) failures.push('Celestial control must align without covering edit')
        return failures
      }),
    }
  ...
  try {
    ...
    for (const [width, height] of dimensions) {
      ...
      for (const count of [0, 1, 2, 3, 30]) {
        const preferences = {
          version: 1, hourCycle: '24', theme: height > 760 ? 'light' : 'dark', language: 'es',
      ...
      await page.getByRole('button', { name: 'React Theme', exact: true }).click()
    ...
    expect(longCardSize!.height).toBeCloseTo(300, 0)
    await added.locator('.card-offset').scrollIntoViewIfNeeded()
